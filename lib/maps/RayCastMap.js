	...
	RayCastMap = Ω.Map.extend({
		...
		castRays: function (entities, p) {
			...
			var i,
				...
				hit,
				hitDist,
				// Wacky view distance effect, depends on player depth
				viewDistance = this.viewDistance / (Ω.utils.lerpPerc(1, 8, p.depth)),
				shaded = false;
  			...
  			for (i = 0; i < this.numRays; i++) {
				...
				if (hit) {
					...
					// Fix fish-eye, and then go crazy for wobbly effect
					hitDist = hitDist * Math.cos(rayAngle) / Math.max(1, p.depth * 1);

					var height = viewDistance / hitDist | 0,
						width = (viewDistance / hitDist | 0) * this.stripWidth,
						top = (Ω.env.h - height) / 2,

						// Some craziness for colour saturation effect
						planeDepth = 100 - Math.min(100, (Math.abs(hitDist) + 1) * 13 | 0),
						hitC = hit.cell || 10,
						sat = Ω.utils.lerpPerc(40, 0, p.depth),
						texX = null,
						texY = null,
						col = "#000";

					// Check if strip depth not too far away
					if (planeDepth > 1) {

						if (!hit.shaded) {
							planeDepth = Math.max(0, planeDepth * 0.7);
						}
						...
						col = "hsl(" + (hitC * 30) + ", " + sat + "%, " + planeDepth + "%)";
					}
